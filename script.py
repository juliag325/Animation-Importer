...
def SIP_ExportFBXCharacter(exportNode):
    ...
    for curExportNode in exportNodes:
        if cmds.getAttr(curExportNode + ".export"):
            ...
            meshes = cmds.listConnections(exportNode + ".exportMeshes")
...
def SIP_FBXExporterUI_UpdateModelExportSettings():
    exportNodes = cmds.textScrollList("sip_FBXExporter_window_modelsExportNodesTextScrollList", query = True, selectItem = True)
    ...
    if exportNodes:
        cmds.textFieldButtonGrp("sip_FBXExporter_window_modelExportFileNameTextFieldButtonGrp", edit = True, text = cmds.getAttr(exportNodes[0] + ".exportName"))
        cmds.checkBoxGrp("sip_FBXExporter_window_modelExportCheckBoxGrp", edit = True, enable =  True, value1 = cmds.getAttr(exportNodes[0] + ".export"))
...
#PURPOSE        Populate the Animation Export Nodes textScrollList with export nodes connected to the origin of the character
#               selected in the actorsTextScrollList
#PROCEDURE      get the selected actor's namespace from actorsTextScrollList
#               if valid, get origin with SIP_ReturnFBXExportNodes
#               if origin is valid, get export nodes with SIP_ReturnFBXExportNodes
            ...
            if exportNodes:
                for cur in exportNodes:
                    ...
                    if not testForMeshes:
                        cmds.textScrollList("sip_FBXExporter_window_animationExportNodesTextScrollList", edit = True, append = cur)
...
#PURPOSE        Unlock the UI elements on the animation tab and set them according to attributes on selected export node
#PROCEDURE      get the selected export node from the exportNodesTextScrollList
#               we will querry its attributes and set the ui elements based on those values.
#PRESUMPTION    exportNode is a valid object
def SIP_FBXExporterUI_UpdateAnimationExportSettings():
    exportNodes = cmds.textScrollList("sip_FBXExporter_window_animationExportNodesTextScrollList", query = True, selectItem = True)
    
    if exportNodes:
        SIP_AddFBXNodeAttrs(exportNodes[0])

        cmds.checkBoxGrp("sip_FBXExporter_window_animationExportCheckBoxGrp", edit = True, enable = True, value1 = cmds.getAttr(exportNodes[0] + ".export"))
        cmds.checkBoxGrp("sip_FBXExporter_window_animationZeroOriginCheckBoxGrp", edit = True, enable = True, value1 = cmds.getAttr(exportNodes[0] + ".moveToOrigin"))    
        cmds.checkBoxGrp("sip_FBXExporter_window_animationSubRangeCheckBoxGrp", edit = True, enable = True, value1 = cmds.getAttr(exportNodes[0] + ".useSubRange"))
        
        if cmds.getAttr(exportNodes[0] + ".useSubRange"):
            cmds.floatFieldGrp("sip_FBXExporter_window_animationStartFrameFloatFieldGrp", edit = True, enable = True, value1 = cmds.getAttr(exportNodes[0] + ".startFrame"))    
            cmds.floatFieldGrp("sip_FBXExporter_window_animationEndFrameFloatFieldGrp", edit = True, enable = True, value1 = cmds.getAttr(exportNodes[0] + ".endFrame"))
        else:
        ...
        if cmds.getAttr(exportNodes[0] + ".moveToOrigin"):
            cmds.checkBoxGrp("sip_FBXExporter_window_animationZeroOriginMotionCheckBoxGrp", edit = True, enable = True, value1 = cmds.getAttr(exportNodes[0] + ".zeroOrigin"))
        else:
        ...
        if test:
            cmds.button("sip_FBXExporter_window_animationRecordAnimLayersButton", edit = True, enable = True, label = "Re-Record Anim Layers", backgroundColor = [0.25, 0.25, 1.0])
        else:
            cmds.button("sip_FBXExporter_window_animationRecordAnimLayersButton", edit = True, enable = True, label = "Record Anim Layers", backgroundColor = [1.0, 0.25, 0.25])
        ...
        cmds.textFieldButtonGrp("sip_FBXExporter_window_animationExportFileNameTextFieldButtonGrp", edit = True, enable = True, text = cmds.getAttr(exportNodes[0] + ".exportName"))
...
#                setAttr according to values queried in ui
#PRESUMPTION     none
def SIP_FBXExporterUI_UpdateExportNodeFromAnimationSettings():
    ...
    if exportNodes and cmds.objExists(exportNodes[0]):
        ...
        cmds.setAttr(exportNodes[0] + ".export", cmds.checkBoxGrp("sip_FBXExporter_window_animationExportCheckBoxGrp", query = True, value1 = True))
        cmds.setAttr(exportNodes[0] + ".moveToOrigin", cmds.checkBoxGrp("sip_FBXExporter_window_animationZeroOriginCheckBoxGrp", query = True, value1 = True))
        cmds.setAttr(exportNodes[0] + ".zeroOrigin", cmds.checkBoxGrp("sip_FBXExporter_window_animationZeroOriginMotionCheckBoxGrp", query = True, value1 = True))
        cmds.setAttr(exportNodes[0] + ".useSubRange", cmds.checkBoxGrp("sip_FBXExporter_window_animationSubRangeCheckBoxGrp", query = True, value1 = True))

        if cmds.getAttr(exportNodes[0] + ".useSubRange"):
            cmds.setAttr(exportNodes[0] + ".startFrame", cmds.floatFieldGrp("sip_FBXExporter_window_animationStartFrameFloatFieldGrp", query = True, value1 = True))
            cmds.setAttr(exportNodes[0] + ".endFrame", cmds.floatFieldGrp("sip_FBXExporter_window_animationEndFrameFloatFieldGrp", query = True, value1 = True))
            
        cmds.setAttr(exportNodes[0] + ".exportName", cmds.textFieldButtonGrp("sip_FBXExporter_window_animationExportFileNameTextFieldButtonGrp", query = True, text = True), type = "string")




#PURPOSE          Create a new export node and connect to the origin of the character selected in actorsTextScrollList
#PROCEDURE        get the name of the actor's namespace by querying the actorsTextScrollList.
#                 we will get the origin using SIP_ReturnOrigin
#                 we then create the export node with SIP_CreateExportNode
#                 we then connect to the origin with SIP_ConnectFBXExportNodeToOrigin
#                 we then repopulate the ui        
#PRESUMPTION      none
...
#PROCEDURE      get selected export node from UI  
#               call SIP_SetAnimLayerSettings
#               reset the button 
...
def SIP_FBXExporterUI_RecordAnimLayers():
    ...
    if exportNodes and cmds.objExists(exportNodes[0]):
        ...
        cmds.button("sip_FBXExporter_window_animationRecordAnimLayersButton", edit = True, label = "Re-Record Anim Layers", backgroundColor = [0.250, 0.25, 1.25])
...
#PROCEDURE      get selected export node from UI
#               call SIP_ClearAnimLayersFromSetting
...
def SIP_FBXExporterUI_ClearAnimLayers():
    ...
    if exportNodes and cmds.objExists(exportNodes[0]):
        ...
        cmds.button("sip_FBXExporter_window_animationRecordAnimLayersButton", edit = True, label = "Record Anim Layers", backgroundColor = [1.0, 0.25, 0.25])
...
def SIP_FBXExporterUI_RenameExportNode_UI(uiElement):
    ...
    cmds.formLayout("sip_FBXExporter_rename_formLayout", edit = True, attachForm = [("sip_FBXExporter_rename_textFieldGrp", 'top', 5), ("sip_FBXExporter_rename_textFieldGrp", 'left', 5), ("sip_FBXExporter_rename_renameButton", 'left',50)])
    cmds.formLayout("sip_FBXExporter_rename_formLayout", edit = True, attachControl = [("sip_FBXExporter_rename_renameButton", 'top', 10, "sip_FBXExporter_rename_textFieldGrp"), ("sip_FBXExporter_rename_cancelButton", 'top', 10, "sip_FBXExporter_rename_textFieldGrp"), ("sip_FBXExporter_rename_cancelButton", 'left', 50, "sip_FBXExporter_rename_renameButton")])
...
def SIP_FBXExporter_UI():
    ...
    cmds.checkBoxGrp("sip_FBXExporter_window_animationExportCheckBoxGrp", numberOfCheckBoxes = 1, label = "Export", columnWidth2 = [85, 70], enable = False, cc = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_UpdateExportNodeFromAnimationSettings()",  parent = "sip_FBXExporter_window_animationFormLayout")
    cmds.checkBoxGrp("sip_FBXExporter_window_animationZeroOriginCheckBoxGrp", numberOfCheckBoxes = 1, label = "Move to Origin", columnWidth2 = [85, 70], enable = False, cc = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_UpdateExportNodeFromAnimationSettings()\nFBX.SIP_FBXExporterUI_UpdateAnimationExportSettings()", parent = "sip_FBXExporter_window_animationFormLayout")
    cmds.checkBoxGrp("sip_FBXExporter_window_animationZeroOriginMotionCheckBoxGrp", numberOfCheckBoxes = 1, label = "Zero Motion on Origin", columnWidth2 = [120, 70], enable = False, cc = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_UpdateExportNodeFromAnimationSettings()", parent = "sip_FBXExporter_window_animationFormLayout")
    cmds.checkBoxGrp("sip_FBXExporter_window_animationSubRangeCheckBoxGrp", numberOfCheckBoxes = 1, label = "Use Sub Range", columnWidth2 = [85, 70], enable = False, cc = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_UpdateExportNodeFromAnimationSettings()\nFBX.SIP_FBXExporterUI_UpdateAnimationExportSettings()", parent = "sip_FBXExporter_window_animationFormLayout")
    cmds.floatFieldGrp("sip_FBXExporter_window_animationStartFrameFloatFieldGrp", numberOfFields=1, label = 'Start Frame', columnWidth2 = [75,70], enable = False, value1 = 0.0, cc = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_UpdateExportNodeFromAnimationSettings()", parent = "sip_FBXExporter_window_animationFormLayout")
    cmds.floatFieldGrp("sip_FBXExporter_window_animationEndFrameFloatFieldGrp", numberOfFields=1, label = 'EndFrame', columnWidth2 = [75,70], enable = False, value1 = 1.0, cc = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_UpdateExportNodeFromAnimationSettings()", parent = "sip_FBXExporter_window_animationFormLayout")
    cmds.textFieldButtonGrp("sip_FBXExporter_window_animationExportFileNameTextFieldButtonGrp", label = "Export File Name", columnWidth3 = [100,300,30], enable = False, text = '', buttonLabel = 'Browse', bc="import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_BrowseExportFilename(1)", cc = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_UpdateExportNodeFromAnimationSettings()", parent = "sip_FBXExporter_window_animationFormLayout")    
    cmds.button("sip_FBXExporter_window_animationRecordAnimLayersButton", enable = False, width = 150, height = 50, label = "Record Anim Layers", c = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_RecordAnimLayers()", backgroundColor = [1, .25, .25], parent = "sip_FBXExporter_window_animationFormLayout")
    cmds.button("sip_FBXExporter_window_animationPreviewAnimLayersButton", enable = False, width = 250, height = 50, label = "Preview Anim Layers", c = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_PreviewAnimLayers()", parent = "sip_FBXExporter_window_animationFormLayout")
    cmds.button("sip_FBXExporter_window_animationClearAnimLayersButton", enable = False, width = 250, height = 50, label = "Clear Anim Layers", c = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_ClearAnimLayers()", parent = "sip_FBXExporter_window_animationFormLayout")
    ...
    cmds.button("sip_FBXExporter_window_animationExportSelectedAnimationButton", width = 300, height = 50, label = "Export Selected Animation", c = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_ExportSelectedAnimation()", parent = "sip_FBXExporter_window_animationFormLayout")
    cmds.button("sip_FBXExporter_window_animationExportAllAnimationsForSelectedCharacterButton", width = 300, height = 50, c = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_ExportAllAnimationFromSelectedCharacter()", label = "Export All Animation for Selected Charater", parent = "sip_FBXExporter_window_animationFormLayout")
    cmds.button("sip_FBXExporter_window_animationExportAllAnimationsButton", width = 300, height = 50, label = "Export All Animations", c = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_ExportAllAnimation()", parent = "sip_FBXExporter_window_animationFormLayout")
    ...
    cmds.menuItem("sip_FBXExporter_window_animationSelectNodeMenuItem", label = "Select", command = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_SelectExportNode(\"sip_FBXExporter_window_modelsExportNodesTextScrollList\")",parent = "sip_FBXExporter_window_animationExportNodesPopupMenu" )
    cmds.menuItem("sip_FBXExporter_window_animationRenameNodeMenuItem", label = "Rename", command = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_RenameExportNode_UI(\"sip_FBXExporter_window_animationExportNodesTextScrollList\")", parent = "sip_FBXExporter_window_animationExportNodesPopupMenu" )
    cmds.menuItem("sip_FBXExporter_window_animationDeleteNodeMenuItem", label = "Delete", command = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_DeleteExportNode(\"sip_FBXExporter_window_animationExportNodesTextScrollList\")", parent = "sip_FBXExporter_window_animationExportNodesPopupMenu" )
    ...
    cmds.menuItem("sip_FBXExporter_window_modelSelectNodeMenuItem", label = "Select", command = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_SelectExportNode(\"sip_FBXExporter_window_modelsExportNodesTextScrollList\")", parent = "sip_FBXExporter_window_modelExportNodesPopupMenu" )
    cmds.menuItem("sip_FBXExporter_window_modelRenameNodeMenuItem", label = "Rename", command = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_RenameExportNode_UI(\"sip_FBXExporter_window_modelsExportNodesTextScrollList\")", parent = "sip_FBXExporter_window_modelExportNodesPopupMenu" )
    cmds.menuItem("sip_FBXExporter_window_modelDeleteNodeMenuItem", label = "Delete", command = "import SIP_FBXAnimationExporter as FBX\nFBX.SIP_FBXExporterUI_DeleteExportNode(\"sip_FBXExporter_window_modelsExportNodesTextScrollList\")", parent = "sip_FBXExporter_window_modelExportNodesPopupMenu" )
